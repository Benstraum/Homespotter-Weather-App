...
function LocationQuery(props) {
  ...
  return (
      <div className="locationQuery" style={{display:'inline-flex'}}>
        <input placeholder="location" value={location} onChange={e=>setLocation(e.target.value)}/>
        {/* Seeing that google had SVGS created for clear and search I thought to use those values instead of using an icon library */}
        <button className="clear-button" onClick={()=>setLocation('')}><svg xmlns="http://www.w3.org/2000/svg">
            <path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/></svg>
         </button>
        <span className="divider"/>
        <button className="search" onClick={()=>sendLocationInfo(location)}>
